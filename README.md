...
> **Note on the log files:** The sample logs in `data/` are generated fresh
> by `generate_logs.py` when your Codespace is created, so the dates always
> look recent. If `data/` looks empty, just run `python generate_logs.py`
> once to create them.
...
|---|---|
| `data/` | Lab log files — generated fresh by `generate_logs.py` (synthetic, safe) |
